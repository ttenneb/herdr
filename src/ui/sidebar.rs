...
pub(crate) fn top_level_space_targets(app: &AppState) -> Vec<crate::app::state::SpaceRowTarget> {
    ...
    app.space_order
        ...
        .filter_map(|space| match space {
            crate::repository::SpaceRef::Repository(id) => {
                repository_root_workspace_index(app, id)?;
                Some(crate::app::state::SpaceRowTarget::Repository(id.clone()))
            }
            crate::repository::SpaceRef::StandaloneWorkspace(id) => app
                .workspaces
                .iter()
    ...
    for card in cards {
        ...
        } = &card.target
        {
            ...
            if resource_selected || owner_selected || owner_active {
                ...
                let bg = if resource_selected || owner_selected {
                    workspace_selection_background(p, owner_active)
                } else {
                    p.active_row_bg
                };
                let buf = frame.buffer_mut();
...
"##,
    ...
    #[test]
    fn collapsed_sidebar_repository_aggregates_linked_attention_and_selection() {
        let mut app = app_with_blocked_linked_checkout();
        app.palette = crate::app::state::Palette::catppuccin();
        app.mode = Mode::Navigate;
        app.selected_repository_id = Some("repo".into());
        ...
        assert_eq!(
            buffer[(spaces.x + 2, spaces.y)].style().fg,
            Some(app.palette.red)
        );
        assert_eq!(
            buffer[(spaces.x, spaces.y)].style().bg,
            Some(app.palette.selection_bg)
        );
        assert_eq!(
            top_level_space_targets(&app),
            vec![crate::app::state::SpaceRowTarget::Repository("repo".into())]
        );
    }

    #[test]
    fn expanded_repository_row_uses_active_and_selection_backgrounds() {
        let mut app = app_with_blocked_linked_checkout();
        app.palette = crate::app::state::Palette::catppuccin();
        app.active = Some(1);
        app.mode = Mode::Terminal;
        let area = Rect::new(0, 0, 30, 10);
        let repository_row = 1;
        app.view.workspace_card_areas = vec![crate::app::state::WorkspaceCardArea {
            target: crate::app::state::SpaceRowTarget::Repository("repo".into()),
            rect: Rect::new(area.x, repository_row, area.width, 1),
            depth: 0,
            indented: false,
        }];
        ...
        let render = |app: &AppState| {
            ...
            terminal
                ...
                .unwrap();
            terminal.backend().buffer()[(area.x, repository_row)]
                .style()
                .bg
        };

        assert_eq!(render(&app), Some(app.palette.active_row_bg));
        app.mode = Mode::Navigate;
        app.selected_repository_id = Some("repo".into());
        assert_eq!(render(&app), Some(app.palette.selection_bg));
    }
    ...
    fn active_checkout_highlight_extends_through_its_resource_rows() {
        ...
        for resource_row in &resource_rows {
            assert_eq!(
                buffer[(list_area.x, *resource_row)].style().bg,
                Some(if *resource_row == resource_rows[1] {
                    app.palette.selection_bg
                } else {
                    app.palette.active_row_bg
                })
