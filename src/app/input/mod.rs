...
impl App {
    ...
    pub(super) fn handle_mouse_from_input_source(
        ...
        let host_drag_continuation = self.state.drag_continues_for_input_source(source_id)
            && matches!(
                mouse.kind,
    ...
    fn handle_modified_url_click_with(
        ...
        self.handle_modified_url_click_at_with(
            source_id,
            mouse,
            PaneUrlClickTarget {
                pane_id: info.id,
    ...
    fn handle_modified_url_click_at(
        ...
        self.handle_modified_url_click_at_with(source_id, mouse, target, crate::platform::open_url)
    }

    fn handle_modified_url_click_at_with(
        &mut self,
        source_id: super::InputSourceId,
        mouse: MouseEvent,
        target: PaneUrlClickTarget,
        ...
        let plugin_handled = match self.invoke_plugin_link_handler_for_url(&url, target.pane_id) {
            Ok(handled) => handled,
            Err(err) => {
