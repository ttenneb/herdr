...
pub fn git_status_snapshot_for_cwd_with_demand(
    ...
    let repository_info = repository_context.0.clone();
    if !demand.ahead_behind {
        ...
        let upstream = demand
            .upstream
            .then(|| primary_remote_upstream_name(&repository_info))
            .flatten();
        let snapshot = WorkspaceGitStatusSnapshot {
            auto_label,
            branch,
...
fn primary_remote_upstream_name(info: &GitWorktreeInfo) -> Option<String> {
    ...
    let repo_root = if info.is_linked_worktree {
        (info.git_common_dir.file_name()? == ".git")
            .then(|| info.git_common_dir.parent().map(Path::to_path_buf))
            .flatten()?
    } else {
        info.repo_root.clone()
    };
    let mut repo = repo_context(&repo_root)?;
    let branch = match read_head_identity(&repo.0, repo.1)? {
        GitHeadIdentity::Branch { short_name, .. } => short_name,
        GitHeadIdentity::Detached { .. } => return None,
    };
    read_upstream(&mut repo, &branch)?.remote_name()
}
