...
| Collection | Place several real panes in one scrollable layout leaf. |
| Delegation | Record parent, purpose, and sibling order independently of placement. |
...
`idle` means the agent is ready for input and has been seen by a foreground human. `done` is the same underlying idle state after unseen work. A tiled pane becomes seen when its tab is actually viewed in the foreground Herdr UI. CLI `pane focus`, `agent focus`, reads, and waits do not acknowledge it. `blocked` means Herdr recognized an approval or question UI. `unknown` means an agent is present but Herdr cannot classify its lifecycle confidently; it does not prove successful completion. Use exact `--until` states when that distinction matters.
...
## Grouped agent orchestration

Use a collection to avoid one top-level split per helper, and delegation to retain parentage across moves and archival.

```bash
parent=$(herdr delegation create --pane "$HERDR_PANE_ID" --purpose primary)
parent_id=$(printf '%s\n' "$parent" | jq -r '.result.delegation.delegation_id')
created=$(herdr collection create --target-pane "$HERDR_PANE_ID" --direction right --label helpers --no-focus)
collection_id=$(printf '%s\n' "$created" | jq -r '.result.collection.collection_id')
child=$(herdr collection member-create "$collection_id" --cwd "$PWD" --parent "$parent_id" --purpose review)
child_pane=$(printf '%s\n' "$child" | jq -r '.result.created.pane.pane_id')
herdr agent start reviewer --kind codex --pane "$child_pane"
```

Reuse the collection. Do not infer success or result delivery from `done`, `idle`, or exit; archive explicitly only after validating the result. Reads, waits, selection, API focus, and observation do not acknowledge grouped children. On removal, use `--promote-members` to preserve processes or `--cascade-close` to close every PTY. Never cascade-close merely because agents look idle or done.
