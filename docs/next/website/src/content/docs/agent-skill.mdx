...
- organize helper PTYs in one collection without stealing focus
- record delegation parentage and purpose independently of placement
- preserve human attention and choose explicit archive or close outcomes
...
The command guide lives in the skill file itself. It covers pane IDs, `pane split`, `pane run`, `pane read`, `pane wait-output`, `agent wait`, workspace and tab commands, and coordination recipes.

For collections, also follow [Grouped agent orchestration](/docs/agent-automation/#grouped-agent-orchestration): reuse a helper collection, create provenance explicitly, preserve focus, do not treat `done`/`idle` as proof of success, archive only after handling results, and promote rather than cascade-close when terminals should survive. The installed binary is authoritative for exact `collection` and `delegation` syntax.
