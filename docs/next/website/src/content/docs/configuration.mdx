...
These warn when an archive member reaches the configured age, archived membership exceeds the configured count, or combined `working` and `blocked` membership exceeds the configured concurrency. Set an individual value to `0` to disable that warning.

The thresholds are advisory. They never archive, stop, close, or reject input. Archive cleanup and cascade-close remain explicit confirmed actions; input or renewed `working`/`blocked` state restores an archived member.
