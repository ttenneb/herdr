...
agent, selected text, clicked URL, link handler, and `workspace_resource`
fields when they are available for that invocation. Shell plugins can read the individual env vars
...
An action can populate Herdr's native context menu asynchronously by declaring a
second argv command:
...
command = ["node", "open-target.js"]
choices_command = ["node", "list-targets.js"]
```

`choices_command` is optional and, like `command`, must be a nonempty argv array.
It is eligible only when a workspace, tab, pane, or collection-member context
menu matches one of the action's `contexts`; collection-member menus use the
member's `pane` context. `global` and `selection` do not provide choices. The
current choices consumer is the native TUI context menu. There is no public
socket request for resolving action choices.

The provider must write one strict UTF-8 JSON document to stdout. Version 1 has
this exact shape; all fields are required and unknown fields are rejected:
...
`payload` may be any JSON value. Choice ids must be nonempty and unique within
one provider result. Provider order is preserved. The fixed version-1 limits
are:

- 2 seconds per provider, 4 providers running globally, and 8 eligible
  providers per menu;
- 64 choices per provider and 128 choices in the combined menu;
- 64 KiB stdout and 16 KiB retained stderr per provider;
- 120 Unicode characters per choice id and 256 per label;
- 8 KiB for each payload after JSON serialization and 16 levels of array/object
  nesting.

Labels must have visible terminal width. Empty or zero-width-only labels,
control characters (including terminal controls, newlines, and tabs), and
Unicode bidi formatting or override characters are rejected. Output above a
limit, a nonzero exit, a timeout, malformed JSON, a trailing JSON document,
duplicate ids, or any invalid choice rejects that provider's entire result.

Herdr captures the invocation context when the target menu opens. A workspace
menu captures its active tab and focused pane, a tab menu captures that tab and
its focused pane, and pane and collection-member menus capture the targeted
pane (the collection member for the latter). The compact
`HERDR_PLUGIN_CONTEXT_JSON` supplied unchanged to both the provider and the
selected action contains the available fields from this exact snapshot:
`workspace_id`, `workspace_label`, `workspace_cwd`, `worktree` (`repo_key`,
`repo_name`, `repo_root`, `checkout_path`, and `is_linked_worktree`), `tab_id`,
`tab_label`, `focused_pane_id`, `focused_pane_cwd`, `focused_pane_agent`,
`focused_pane_status`, and `selected_text`. It also contains
`invocation_source: "context_menu"` and a `correlation_id` of the form
`context-menu-<generation>`; unavailable optional fields, including
`clicked_url` and `link_handler_id`, are omitted. Focus, cwd, agent state, or
selection changes while the menu is open do not alter this captured JSON.

### Workspace resources

Plugins can report transient runtime children beneath a Checkout or standalone
Workspace with `workspace.resources.report` (or `herdr workspace
report-resources`). A report replaces the reporting plugin's complete resource
set for one Workspace atomically. The JSON array contains `resource_id`,
`label`, optional `detail`, and optional `data`; resource IDs are opaque stable
live identities. Herdr permits at most 32 resources from one plugin, 64 total
resources and 32 reporting plugins per Workspace. IDs are capped at 120
characters, visible text at 256 characters, and `data` at 8 KiB/16 nesting
levels. `ttl_ms` expires a complete report set; resources are transient and are
not persisted in session state.

Actions may declare `contexts = ["workspace_resource"]`. A resource menu has
no native actions and is offered only to the plugin that owns that resource;
Repository menus never run plugin providers. The captured
`PluginInvocationContext.workspace_resource` contains the owning
`workspace_id`, `plugin_id`, `resource_id`, `label`, `detail`, and `data`.
Herdr revalidates the Workspace, owner plugin, action context, and the complete
captured resource value before starting a provider and before invoking a
choice. If a replacement changed the same resource ID's label, detail, or data,
the old menu is stale and must be reopened.

Providers run off the UI event loop. Herdr shows one loading row until every
eligible provider has settled, then publishes results deterministically in
bytewise `(plugin_id, action_id)` order while preserving each provider's choice
order. Providers beyond the concurrency limit queue. A failed provider adds no
row and reports only through the normal provider logs; successful providers
remain usable after partial failure. Inspect them with `herdr plugin log list`,
which records the provider command, status, capped stdout and stderr, and error.

Selecting a choice runs the action's original `command` argv unchanged and adds
`HERDR_PLUGIN_ACTION_CHOICE_JSON`, containing the exact selected object:

```json
{"id":"target-1","label":"Open target 1","payload":{"target":1}}
```

Herdr revalidates the target and the exact linked, enabled, platform-compatible
plugin/action manifest before starting queued providers and again before
invocation. Closing, reopening, or retargeting the menu, duplicate or late
completion, unlinking or disabling the plugin, or changing/relinking its
manifest makes old results stale; stale results are ignored. Selection is
refused with safe feedback when the target or action is no longer valid, and a
changed action asks the user to reopen the menu instead of invoking it.

Direct invocation, including `herdr plugin action invoke` and action
keybindings, runs `command` immediately: it does not run `choices_command` and
does not set `HERDR_PLUGIN_ACTION_CHOICE_JSON`. Herdr also scrubs inherited
plugin-routing and choice variables before launching either command.

Choices providers are trusted plugin code, just like other plugin commands.
Herdr bounds their output and runtime and uses platform process-tree cleanup so
timeouts and descendants do not normally leak resources, but this hygiene is
not a sandbox or security boundary. A plugin can still run arbitrary code or
deliberately escape that cleanup.
