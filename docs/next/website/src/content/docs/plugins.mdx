...
It is eligible only when a workspace, tab, or pane context menu matches one of
the action's `contexts`; `global` and `selection` do not provide choices. The
...
its focused pane, and a pane menu captures the targeted pane. The compact
