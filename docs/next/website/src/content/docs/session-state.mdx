...
If the Herdr server stops and starts again, the original pane processes are gone. Herdr restores the saved session shape: the ordered main-worktree-root/standalone Space list, Checkout membership, workspaces, tabs, panes, cwd, layout, and focus. Existing workspace, tab, pane, and terminal identities are preserved across the repository-state migration. Missing checkout directories drop only that Checkout; an empty Repository is removed.
