...
メンバーは通常のペインと PTY のままです。`member-create` の `.result.created.pane.pane_id` を `agent start --pane` に使います。選択や API フォーカスは確認済みにしません。アーカイブは入力または `working` / `blocked` で復元され、制限は警告だけです。空でないクローズには `--cascade-close` または `--promote-members` が必須です。
