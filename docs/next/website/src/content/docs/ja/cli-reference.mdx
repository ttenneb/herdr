...
メンバーは通常のペインと PTY のままです。`member-create` の `.result.created.pane.pane_id` を `agent start --pane` に使います。選択や API フォーカスは確認済みにしません。アーカイブは入力または `working` / `blocked` で復元され、制限は警告だけです。空でないクローズには `--cascade-close` または `--promote-members` が必須です。`--promote-members` は各プロセスを同じワークスペース内の独立したタブに移し、`--focus-promoted` は最初のメンバーにフォーカスします。
...
デリゲーションは `d1` 形式の安定した来歴です。親更新は欠落、自分自身、循環を拒否します。`reorder` は兄弟順、`reparent` は関係を変更します。親クローズ後も子孫がある間は tombstone が残ります。正確な socket スキーマは `herdr api schema --json` を参照してください。
...
`idle` は確認済み、`done` は未確認の同じ基礎的な idle 状態です。通常ペインは、そのタブがフォアグラウンドの Herdr UI で実際に表示されたときだけ確認済みになります。CLI の `pane focus` と `agent focus` はフォーカスを変えますが、確認済みにはしません。コレクションメンバーは人間が子ターミナルへ入ったときだけ確認済みになります。CLI の読み取りや待機でも変わりません。`unknown` は成功を意味しません。
