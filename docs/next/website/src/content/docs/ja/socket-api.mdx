...
`TabInfo`、`WorkspaceInfo`、`RepositoryInfo` には `descendant_attention_count` が含まれます。`agent_status` はトップレベルのエージェントに従い、このカウントは、トップレベルの親ターミナルへの入力が正常に受理されるまで未確認の完了済み委任子孫を表します。親ペインが存続している委任子への入力ではカウントはクリアされません。親ペインが閉じられた場合、残った子は確認上の実効ルートになります。ブロック中の子孫は引き続き集約ステータスを `blocked` にします。
