...
| リポジトリ | `repository.list`、`repository.get`、`repository.focus`、`repository.rename`、`repository.move`、`repository.close` |
| チェックアウト | `checkout.open`、`checkout.focus`、`checkout.rename`、`checkout.move`、`checkout.close` |
| ワークスペース互換 | `workspace.create`、`workspace.list`、`workspace.get`、`workspace.focus`、`workspace.rename`、`workspace.move`、`workspace.move_block`、`workspace.report_metadata`、`workspace.resources.report`、`workspace.close` |
...
| コレクション | `collection.list`、`collection.get`、`collection.create`、`collection.add`、`collection.move`、`collection.promote`、`collection.select`、`collection.reorder`、`collection.archive`、`collection.restore`、`collection.member_create`、`collection.close` |
| 委譲 | `delegation.create`、`delegation.get`、`delegation.tree`、`delegation.root`、`delegation.descendants`、`delegation.reparent`、`delegation.reorder` |
| ポップアップ | `popup.close` |
| レイアウト | `layout.export`、`layout.apply`、`layout.set_split_ratio` |
| エージェント | `agent.list`、`agent.get`、`agent.read`、`agent.explain`、`agent.send_keys`、`agent.prompt`、`agent.wait`、`agent.rename`、`agent.focus`、`agent.start`、`agent.view.set`、`agent.view.clear` |
...
`agent.wait` はサーバー所有でイベント駆動です。解決したペイン占有者に固定されるため、置換されたエージェントが待機を満たすことはありません。`agent.prompt` は `until` と `timeout_ms` を持つ省略可能な `wait` オブジェクトを受け付けます。これにより、プロンプト送信と待機開始を 1 つのリクエストで行い、別々の呼び出し間の競合を避けられます。解決したエージェントがすでに `blocked` の場合、`agent.prompt` は入力を送信せず、待機も開始せずに `agent_blocked` を返します。成功した `agent.prompt` は、現在の端末ランタイムが直列化された text → delay → Enter トランザクションを受理したことを意味し、PTY への最終的な配信を保証するものではありません。各端末は、アクティブ分を含め最大 8 件、合計 64 KiB のプロンプトトランザクションを受理します。`agent_prompt_queue_full` の場合はランタイムの処理完了後に再試行してください。`agent_prompt_payload_too_large` は単一のトランザクションが 64 KiB を超えることを示します。ランタイムの終了または置換は保留中のトランザクションを取り消し、置換先へは書き込みません。
...
`session.snapshot` は、独自のローカルランタイムキャッシュを持つクライアント向けに、一度限りのブートストラップスナップショットを返します。レスポンスには、バージョン/プロトコルメタデータ、`result.snapshot.sequence` のウォーターマーク、フォーカス中のワークスペース/タブ/ペイン id、ワークスペース、タブ、ペイン、タブレイアウト、エージェントの各レコードが含まれます。これは購読ではありません。ブートストラップ中のイベントを取りこぼさないよう、まず別の接続で `events.subscribe` を開き、`subscription_started` の確認応答を待ちます。その `result.sequence` は、リクエスト内のすべての購読に共通して受理されたセッション全体のカーソルです。そのストリームをバッファしながら `session.snapshot` を呼び出してスナップショットを適用し、トップレベルの `sequence` がスナップショットのウォーターマーク以下であるバッファ済み行を破棄してから、残りを受信順に適用し、そのままストリーミングを続けます。カスタム観測ごとに新しいグローバル値が予約されますが、重複するライフサイクル購読では同じイベント値が繰り返されることがあります。`sequence` を購読ごとのカウンターとして扱ったり、すべてのストリーム行が一意であると仮定したりしないでください。再接続後やキャッシュが古い可能性がある場合は、`session.snapshot` を再度呼び出します。ワークスペースレコードには `repository_id` と `checkout` の出自情報が追加され、従来の `worktree` の出自情報も維持されます。リポジトリレコードは、チェックアウトの順序、優先ベース、MRU フォーカスを保持します。リポジトリ全体の worktree 検出には引き続き `worktree.list` を使います。

CLI の `herdr api snapshot` は、クライアントやエージェントが簡単にブートストラップできるよう、現在の `session.snapshot` レスポンスを JSON で出力します。

### 一時的なワークスペースリソース

プラグインは `workspace.resources.report` を使って、Checkout または単独の Workspace に上限付きの一時的なランタイムリソースを関連付けられます。レポートは、その Workspace に対する 1 つのプラグインの完全なリソースセットをアトミックに置き換えます。空の配列はセットをクリアします。リソースは `WorkspaceInfo` とセッションスナップショットに追加されますが永続化されず、`ttl_ms` で期限切れにできます。
...
シェル連携では、JSON 引数のクォートを避けるために `herdr workspace report-resources w1 --plugin hs.jail --file resources.json --ttl-ms 30000` を使えます（標準入力には `--file -` を使います）。
...
`pane.input.set` は、1 つのペインの `right_click` を `herdr` または `pane` に設定します。デフォルトは `herdr` です。`pane` を指定すると、アプリケーションがターミナルのマウスレポーティングを要求している間、修飾キーなしの右クリック、ホールド、ドラッグを転送します。それ以外の場合は Herdr のペインメニューにフォールバックします。ペイン枠の右クリックは常に Herdr のメニューを開きます。`pane.split` は、新しく作成するペインに同じ省略可能な `right_click` 値を受け付けます。
...
ペイングラフィックスを使うと、プラグインがペイン上に画像データを配置できます。`[experimental].kitty_graphics = true` の場合にのみ利用でき、それ以外ではすべてのペイングラフィックスメソッドが `feature_disabled` を返します。`pane.graphics.info` を明示的に呼ぶと機能検出が有効になり、接続中クライアントのセルサイズ、ファイルフレームのオプション、ピクセル単位のマウス対応、最大 16 レイヤー、`pane_visible` が返されます。`pane_visible` は対象がアクティブなワークスペースとタブにあり、ズームで隠されていない場合だけ true です。短時間の UI モードでは変わりません。

`pane.graphics.set`、`pane.graphics.clear`、`pane.graphics.stream` は省略可能な `layer_id`（デフォルトは `primary`）を受け付けます。set と stream は `z_index` も受け付け、レイヤーは安定した `(z_index, layer_id)` 順に配置されます。各ストリームはそのレイヤーを排他的に所有し、閉じるとレイヤーも削除されます。インラインフレームの形式は `png`、`rgb`、`rgba`、`bgra` で、BGRA は一度だけ所有 RGBA に正規化されます。Herdr はレンダーパスごとに 1 つの画像トランザクションをホストキャッシュへ進めるため、任意のレイヤー集合が集約フレームなしで進行します。ヘッドレス転送では各トランザクションを 32 MiB のワイヤ上限内に保ちますが、ローカルのモノリシック描画にはこの上限を適用しません。
...
フレームを繰り返し送る場合は、`pane.graphics.stream` で専用ソケットを開きます。Herdr が `ok` を返した後、インラインフレームごとに 1 つの JSON ヘッダーと正確に `data_length` バイトの生データを送ります。そのレイヤーへの同時操作は `stream_conflict` を返します。
...
`pane.graphics.info` が `file_frame_transport: "direct-kitty"` を通知した場合、対象となるローカルの Ghostty、kitty、WezTerm クライアントは、不変で非公開の `rgba` または `bgra` ファイルを `file.path`、`sequence`、`revision` とともに送信できます。Direct Kitty ファイル転送はデフォルトの `primary` ページレイヤー専用です。名前付きのセカンダリレイヤーは所有インライン RGBA を使い、BGRA は常にコピー、スウィズルされてインライン描画されます。Herdr はターミナルがファイルを受理した後、または安全な所有インラインフォールバックを導入した後にだけ `pane_graphics_frame_ack` を返します。ファイル転送の失敗が確認されると、そのクライアント接続では direct file が無効になりますが、正確なピクセルマウスは無効になりません。タイムアウトまたはクライアント切断時は、送信元の再利用を確認応答せずストリームを閉じます。モノリシックな `--no-session` モードは高速ファイル転送も正確なピクセルマウスも通知せず、所有インラインフォールバックを使います。

Direct file は常に完全な正規 `width * height * 4` RGBA フレームです。`file_frame_max_bytes` は所有インラインフォールバックの対象にできる上限です。`file_frame_transport` が利用可能な場合、primary レイヤーの RGBA ファイルはより大きい `file_frame_direct_max_bytes` 上限を使えます。フォールバック上限を超えるフレームは、ターミナルが direct 転送を受理した場合にのみ確認応答され、拒否されるとストリームが閉じます。所有インラインフォールバックを使えないフレームのペインが一時的に非表示、または再描画中に配置不能な場合、Herdr は画像を表示せずアップロードし、ペインが再表示されたときに配置を再生します。`file_frame_damage: true` はプロデューサ側の正規リング効率化に省略可能な damage メタデータを受け付けることを示しますが、完全なファイルをコピーまたは表示します。リサイズと完全再描画ではピクセルを再送せず配置を再生します。
...
`layout.export` は移植可能なタブレイアウトツリーを返します。アクティブなタブをエクスポートするには `tab_id` と `pane_id` の両方を省略し、特定のタブなら `tab_id` を、そのペインを含むタブなら `pane_id` を渡します。コレクション葉を 1 つでも含むタブは `layout_contains_collections` で拒否されます。エクスポート前に、各コレクション葉を削除または閉じてください。メンバーがいるコレクションでは、先にメンバーを通常のタイルペインへ昇格または移動できますが、空のコレクション葉はメンバーの昇格では解消できず、削除またはクローズが必要です。セッションスナップショットとライブハンドオフでは、引き続きコレクションが保持されます。
...
4 つの raw worktree メソッドはすべて `trust_repository` を受け付け、省略時のデフォルトは `false` です。Git が選択されたリポジトリを所有権が安全でない、または疑わしいとして拒否した場合を除き、`false` のままにしてください。解決されたリポジトリパスを独立して確認した後、同じリクエストを `"trust_repository": true` で再試行します。信頼はリクエスト単位です。Herdr はそのリクエストの Git サブプロセスだけに `git -c safe.directory=<resolved-repository>` を追加します。信頼を永続化せず、Git 設定を変更せず、他のパスを信頼しません。後続の各リクエストは再び `false` がデフォルトとなり、必要な場合は個別に明示する必要があります。
...
ペインコレクションには `collection.list`、`collection.get`、`collection.create`、
...
`collection.member_create`、`collection.close` を使います。空でないコレクションを閉じる場合は
`disposition` に `cascade_close` または `promote_members` が必要です。`cascade_close` は全メンバーの PTY を終了し、`promote_members` は各プロセスを同じワークスペース内の新しい独立タブに移して保持します。プロモート時は `target_pane_id` を省略してください。レスポンスには安定 ID、順序付きメンバー、
アーカイブ/選択状態、ライフサイクル集計、警告が含まれます。委譲されたメンバーでは `collection.reorder.index` は、移動するサブツリーのルートに要求する最終的な正規表示インデックスです。このインデックスは移動するサブツリーを除外した後に解釈され、移動先は同じ委譲親かつ同じアクティブ/アーカイブ区分にある兄弟サブツリーの境界でなければなりません。親を変更するには `delegation.reparent` を使います。委譲されていないメンバーは、従来どおりメンバーベクターを直接並べ替えます。

委譲メタデータは配置とは独立しています。`delegation.create`、`delegation.get`、
`delegation.tree`、`delegation.root`、`delegation.descendants`、
`delegation.reparent`、`delegation.reorder` で親子関係、目的、導出ルート、兄弟順を扱います。
循環する再親設定は原子的に拒否されます。

`PaneInfo.placement` と各 `PaneLayoutPane.placement` は `tiled`、または
`collection_id`、`member_index`、`archived`、`selected` を持つ `collection` です。
`PaneLayoutSnapshot.collections` はコレクション葉を表し、`focused` はペインまたはコレクションの
型付きフォーカスです。空のコレクションに対応するため `focused_pane_id` は任意です。
移植可能なレイアウトのルートはコレクション葉をサポートしません。`layout.export` はコレクション構造やメンバーを暗黙に失わないよう、空かどうかにかかわらずコレクションを拒否します。エクスポート前にすべてのコレクション葉を削除または閉じる必要があります。メンバーがいるコレクションでは先にメンバーを昇格または移動できますが、空の葉には引き続き削除またはクローズが必要です。スナップショットとセッション永続化は引き続きコレクションに対応します。
...
最初のレスポンスは、`result` 内の `{"type":"subscription_started","sequence":N}` として購読を確認します。`N` は、リクエストされたすべての購読に共通するセッション全体の受理カーソルです。以降のライフサイクルイベントとカスタムイベントの行は、トップレベルにセッション全体の `sequence` を持ち、カーソル順にマージされます。同じ値の場合はリクエスト順が優先されます。プローブに基づくカスタム観測はそれぞれ次のグローバルシーケンスを予約するため、スナップショットのウォーターマークはこれらの観測にも順序を与えます。ライフサイクルイベントの購読は受理カーソルから始まり、それ以前に保持されていたイベントは再生しません。

Herdr はストリームの追いつき用に最新 512 件のライフサイクルイベントを保持します。購読側が遅れ、次に必要なカーソルが利用できなくなった場合、サーバーはコード `resync_required` のエラーレスポンスを書き込み、そのソケットを閉じます。部分的なキャッシュから続行せず、新しい購読を開き、新しい `session.snapshot` を取得して、上記のウォーターマークに基づくブートストラップ手順を繰り返してください。
...
コレクションイベントは `collection.created`、`collection.member_added`、`collection.member_moved`、`collection.member_removed`、`collection.member_promoted`、`collection.member_selected`、`collection.members_reordered`、`collection.member_archived`、`collection.member_restored`、`collection.closed` です。委譲イベントは `delegation.created`、`delegation.reparented`、`delegation.reordered`、`delegation.tombstoned`、`delegation.garbage_collected` です。利用可能な場合は変更後のレコードを含み、削除/終了イベントはキャッシュ破棄用の安定 ID を含みます。
...
`TabInfo`、`WorkspaceInfo`、`RepositoryInfo` には `descendant_attention_count` が含まれます。`agent_status` はトップレベルのエージェントに従い、このカウントは、トップレベルの親ターミナルへの入力が正常に受理されるまで未確認の、ペインコレクション外にある完了済み委任子孫を表します。ペインがコレクションメンバーである間、未確認の完了はそのコレクション内にだけ表示され、上位のステータスと注意カウントから除外されます。タイルペインへ昇格すると、通常の集約動作に戻ります。親ペインが存続している委任子への入力ではカウントはクリアされません。親ペインが閉じられた場合、残った子は確認上の実効ルートになります。ブロック中の子孫は、コレクション内かどうかにかかわらず、引き続き集約ステータスを `blocked` にします。
