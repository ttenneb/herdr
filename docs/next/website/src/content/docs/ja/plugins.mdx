...
プラグインは `workspace.resources.report` で一時的なリソースを報告できます。`workspace_resource` コンテキストは所有プラグインだけに提供され、古いリソース選択は拒否されます。

### アクション選択

`choices_command` は対応するコンテキストメニューで非同期の選択肢を提供します。
