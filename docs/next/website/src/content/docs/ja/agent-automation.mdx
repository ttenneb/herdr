...
| コレクション | 複数の実ペインを 1 つのスクロール可能リーフへ配置する。 |
| デリゲーション | 配置とは独立して親、目的、兄弟順を記録する。 |
...
`idle` は、エージェントが入力待ちで、フォアグラウンドの人間に確認済みであることを示します。`done` は未確認の作業後に使われる同じ基礎的な idle 状態です。通常ペインは、そのタブがフォアグラウンドの Herdr UI で実際に表示されたときだけ確認済みになります。CLI の `pane focus`、`agent focus`、読み取り、待機では確認済みになりません。`blocked` は承認または質問 UI を Herdr が認識した状態です。`unknown` はエージェントが存在するもののライフサイクルを確実に分類できない状態で、成功完了を意味しません。違いが重要なら正確な `--until` を指定してください。
...
## グループ化したエージェントのオーケストレーション

ヘルパーごとの最上位分割を避けるにはコレクション、移動やアーカイブ後も親子関係を残すにはデリゲーションを使います。
...
同じコレクションを再利用し、`done` / `idle` / 終了から成功を推測せず、結果確認後だけ明示的にアーカイブします。読み取り、待機、選択、API フォーカスは子を確認済みにしません。削除時は、プロセスを保つ `--promote-members` か全 PTY を閉じる `--cascade-close` を選び、idle や done だけを理由に連鎖クローズしないでください。
