...
順に、設定日数に達したアーカイブ、設定値を超えたアーカイブ件数、設定値を超えた `working` + `blocked` 件数を警告します。`0` は個別警告を無効化します。しきい値は助言のみで、自動アーカイブ、停止、クローズ、入力拒否を行いません。入力または `working` / `blocked` でアーカイブメンバーは復元されます。
