...
成员仍是普通窗格和 PTY。把 `member-create` 返回的 `.result.created.pane.pane_id` 用于 `agent start --pane`。选择和 API 聚焦不会确认关注。输入或 `working` / `blocked` 会恢复归档;阈值只是警告。关闭非空集合必须使用 `--cascade-close` 或 `--promote-members`。`--promote-members` 会把每个进程移入同一工作区中的独立标签页;`--focus-promoted` 会聚焦第一个成员。
...
委派是 `d1` 形式的稳定来源。父级更新拒绝缺失、自我父级和环。`reorder` 改兄弟顺序,`reparent` 改关系。关闭父窗格后只要后代存在就保留 tombstone。准确 socket 模式见 `herdr api schema --json`。
...
`idle` 表示已确认,`done` 是未确认的相同底层 idle 状态。普通窗格只有在其标签页实际显示于前台 Herdr 界面时才会确认。CLI 的 `pane focus` 和 `agent focus` 只改变焦点,不会确认关注。集合成员只有在人类进入子终端时才确认。CLI 读取和等待不会改变它。`unknown` 不代表成功。
