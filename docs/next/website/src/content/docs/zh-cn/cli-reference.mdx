...
成员仍是普通窗格和 PTY。把 `member-create` 返回的 `.result.created.pane.pane_id` 用于 `agent start --pane`。选择和 API 聚焦不会确认关注。输入或 `working` / `blocked` 会恢复归档;阈值只是警告。关闭非空集合必须使用 `--cascade-close` 或 `--promote-members`。
