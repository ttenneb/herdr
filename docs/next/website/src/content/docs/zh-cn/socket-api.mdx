...
`TabInfo`、`WorkspaceInfo` 和 `RepositoryInfo` 包含 `descendant_attention_count`。其中 `agent_status` 跟随顶层智能体；该计数表示在顶层父终端成功接收输入之前仍未确认的已完成委派后代。若委派子项的父窗格仍然存在，向该子项输入不会清除计数。父窗格关闭后，存活的子项会成为确认语义上的有效根。被阻塞的后代仍会使聚合状态显示为 `blocked`。
