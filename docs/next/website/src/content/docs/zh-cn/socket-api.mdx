...
| 会话 | `session.snapshot` |
| 仓库 | `repository.list`、`repository.get`、`repository.focus`、`repository.rename`、`repository.move`、`repository.close` |
| 检出 | `checkout.open`、`checkout.focus`、`checkout.rename`、`checkout.move`、`checkout.close` |
| 工作区兼容 | `workspace.create`、`workspace.list`、`workspace.get`、`workspace.focus`、`workspace.rename`、`workspace.move`、`workspace.move_block`、`workspace.report_metadata`、`workspace.resources.report`、`workspace.close` |
...
| 集合 | `collection.list`、`collection.get`、`collection.create`、`collection.add`、`collection.move`、`collection.promote`、`collection.select`、`collection.reorder`、`collection.archive`、`collection.restore`、`collection.member_create`、`collection.close` |
| 委派 | `delegation.create`、`delegation.get`、`delegation.tree`、`delegation.root`、`delegation.descendants`、`delegation.reparent`、`delegation.reorder` |
| 弹窗 | `popup.close` |
| 布局 | `layout.export`、`layout.apply`、`layout.set_split_ratio` |
| 智能体 | `agent.list`、`agent.get`、`agent.read`、`agent.explain`、`agent.send_keys`、`agent.prompt`、`agent.wait`、`agent.rename`、`agent.focus`、`agent.start`、`agent.view.set`、`agent.view.clear` |
...
`agent.wait` 由服务器拥有并由事件驱动。它会固定到已解析的窗格占用者,因此替换后的智能体不能满足该等待。`agent.prompt` 接受可选的 `wait` 对象,其中包含 `until` 和 `timeout_ms`;这样可在一个请求中提交提示并开始等待,避免两个独立调用之间的竞态。如果解析出的智能体已经是 `blocked`,`agent.prompt` 不会发送输入或开始等待,而是返回 `agent_blocked`。成功的 `agent.prompt` 仅表示当前终端运行时已接纳串行的 text → delay → Enter 事务,并不保证最终会写入 PTY。每个终端最多接纳 8 个（包括活动和排队）提示事务及其合计 64 KiB 负载；收到 `agent_prompt_queue_full` 后应在运行时排空后重试；`agent_prompt_payload_too_large` 表示单个事务超过 64 KiB。运行时关闭或被替换会取消待处理事务，绝不会将其写入替换后的运行时。
...
`session.snapshot` 为维护本地运行时缓存的客户端返回一次性引导快照。响应包含版本/协议元数据、`result.snapshot.sequence` 水位线、当前聚焦的工作区/标签页/窗格 id、工作区记录、标签页记录、窗格记录、标签页布局快照和智能体记录。它不是订阅。为避免引导期间漏掉事件，应先在另一个连接上打开 `events.subscribe`，并等待 `subscription_started` 确认；其中的 `result.sequence` 是该请求中所有订阅共同接受的会话全局游标。在调用 `session.snapshot` 时缓冲事件流，安装快照，丢弃顶层 `sequence` 小于或等于快照水位线的已缓冲行，再按接收顺序应用其余行并继续接收事件。每个自定义观测都会预留一个新的全局值，而重复的生命周期订阅可能重复同一生命周期事件的值。不要把 `sequence` 当作每个订阅各自的计数器，也不要要求每条事件流记录都唯一。重新连接后或本地缓存可能已过期时，再次调用 `session.snapshot`。工作区记录会新增 `repository_id` 和 `checkout` 来源信息，同时保留旧版 `worktree` 来源信息。仓库记录负责检出顺序、首选基准和 MRU 焦点。完整的仓库 worktree 发现仍使用 `worktree.list`。

CLI 的 `herdr api snapshot` 会把当前 `session.snapshot` 响应输出为 JSON,让客户端和智能体可以简单地完成引导。

### 临时工作区资源

插件可以使用 `workspace.resources.report` 将有界的临时运行时资源附加到 Checkout 或独立 Workspace。报告会原子替换某个插件在该 Workspace 上的完整资源集；空数组会清除它。资源会附加显示在 `WorkspaceInfo` 和会话快照中，但不会持久化，并可通过 `ttl_ms` 过期。
...
Shell 集成可使用 `herdr workspace report-resources w1 --plugin hs.jail --file resources.json --ttl-ms 30000` 避免 JSON 参数引用问题（使用 `--file -` 从标准输入读取）。
...
`pane.input.set` 为单个窗格把 `right_click` 设置为 `herdr` 或 `pane`，默认值为 `herdr`。当应用请求终端鼠标上报时，`pane` 会转发无修饰键的右键点击、按住和拖动手势；否则 Herdr 会回退到窗格菜单。右键点击窗格边框始终会打开 Herdr 菜单。`pane.split` 接受相同的可选 `right_click` 值，用于新建的窗格。
...
窗格图形允许插件在窗格上放置图像数据。它们只在 `[experimental].kitty_graphics = true` 时可用；否则所有窗格图形方法都返回 `feature_disabled`。显式调用 `pane.graphics.info` 会启用能力发现，并返回所连接客户端的单元格尺寸、文件帧选项、像素级鼠标支持、最多 16 个图层以及 `pane_visible`。只有目标位于活动工作区和标签页中且未被缩放隐藏时，`pane_visible` 才为 true；短暂的 UI 模式不会改变它。

`pane.graphics.set`、`pane.graphics.clear` 和 `pane.graphics.stream` 接受可选的 `layer_id`（默认为 `primary`）。set 和 stream 还接受 `z_index`；图层按稳定的 `(z_index, layer_id)` 顺序放置。每个流独占其图层，关闭流时会删除该图层。内联帧支持 `png`、`rgb`、`rgba` 或 `bgra`；BGRA 会一次性规范化为自有 RGBA。Herdr 每个渲染过程向主机缓存推进一个图像事务，因此任意图层集合无需聚合帧也能推进。无头传输会让每个事务保持在 32 MiB 的线路限制内；本地单体渲染不应用该传输限制。
...
要重复发送帧，请用 `pane.graphics.stream` 打开专用 socket。Herdr 回复 `ok` 后，每个内联帧发送一个 JSON 头，再发送恰好 `data_length` 字节的原始数据。对该图层的并发操作会返回 `stream_conflict`。
...
当 `pane.graphics.info` 公告 `file_frame_transport: "direct-kitty"` 时，符合条件的本地 Ghostty、kitty 或 WezTerm 客户端可以提交不可变的私有 `rgba` 或 `bgra` 文件，并附带 `file.path`、`sequence` 和 `revision`。Direct Kitty 文件传输仅用于默认的 `primary` 页面图层；命名的次级图层使用自有内联 RGBA。BGRA 始终会被复制、通道重排并以内联方式渲染。只有终端接受文件，或已安装安全的自有内联回退之后，Herdr 才回复 `pane_graphics_frame_ack`。确认文件传输失败后，该客户端连接会禁用直接文件，但不会禁用精确像素鼠标。超时或客户端丢失会关闭流，且不会确认源可复用。单体 `--no-session` 模式既不公告快速文件传输，也不公告精确像素鼠标，始终使用自有内联回退。

直接文件始终是完整的规范 `width * height * 4` RGBA 帧。`file_frame_max_bytes` 是仍可使用自有内联回退的大小上限。`file_frame_transport` 可用时，主图层 RGBA 文件可使用更大的 `file_frame_direct_max_bytes` 上限。超过回退上限的帧只有在终端接受直接传输时才会获得确认；拒绝会关闭流。如果帧无法使用自有内联回退，而其窗格暂时隐藏或重绘时无法放置，Herdr 会上传图像但不显示，并在窗格再次可见时重放其放置。`file_frame_damage: true` 表示 Herdr 接受可选的 damage 元数据，以提高生产者侧规范环形缓冲区的效率；它仍会复制或呈现完整文件。调整大小和完整重绘会重放放置，而无需重新传输像素。
...
`layout.export` 返回可移植的标签页布局树。省略 `tab_id` 和 `pane_id` 导出活动标签页,传 `tab_id` 导出该标签页,或传 `pane_id` 导出包含该窗格的标签页。包含任何集合叶节点的标签页会以 `layout_contains_collections` 被拒绝。导出前必须移除或关闭每个集合叶节点。对于有成员的集合,可以先将成员提升或移动为普通平铺窗格;空集合叶节点无法通过提升成员解决,仍须移除或关闭。会话快照和实时移交仍会保留集合。
...
四个原始 worktree 方法都接受 `trust_repository`，省略时默认为 `false`。除非 Git 因所有权不安全或可疑而拒绝所选仓库，否则应保持为 `false`。独立验证解析后的仓库路径后，可用 `"trust_repository": true` 重试同一请求。信任仅对该次请求有效：Herdr 只会为该请求的 Git 子进程添加 `git -c safe.directory=<resolved-repository>`。它不会持久化信任、修改 Git 配置或信任其他路径。之后的每个请求都会再次默认使用 `false`，需要时必须分别明确启用。
...
窗格集合使用 `collection.list`、`collection.get`、`collection.create`、
...
`collection.member_create` 和 `collection.close`。关闭非空集合时必须提供
`disposition: "cascade_close"` 或 `"promote_members"`。`cascade_close` 会终止所有成员 PTY;`promote_members` 会保留所有进程,并把每个成员移入同一工作区中的新独立标签页。提升集合时必须省略 `target_pane_id`。集合响应包含稳定 ID、有序成员、归档/选择状态、生命周期计数和提示。对于已委派成员,`collection.reorder.index` 是移动子树根节点所请求的最终规范显示索引。该索引在移除待移动子树后解释,且目标必须是同一委派父项、同一活动/归档分区中的同级子树边界;如需更改父项,请使用 `delegation.reparent`。未委派成员仍直接重排成员向量。

委派元数据独立于布局。使用 `delegation.create`、`delegation.get`、
`delegation.tree`、`delegation.root`、`delegation.descendants`、
`delegation.reparent` 和 `delegation.reorder` 管理父子关系、用途、派生根和同级顺序。
形成循环的重新挂接会被原子拒绝。

`PaneInfo.placement` 与每个 `PaneLayoutPane.placement` 为 `tiled`,或带有
`collection_id`、`member_index`、`archived`、`selected` 的 `collection`。
`PaneLayoutSnapshot.collections` 描述集合叶节点;`focused` 是窗格或集合的类型化焦点。
为支持空集合,`focused_pane_id` 是可选字段。可移植布局根节点不支持集合叶节点。`layout.export` 会拒绝空集合和非空集合,而不会静默丢失集合结构或成员。导出前必须移除或关闭所有集合叶节点;有成员的集合可以先提升或移动成员,而空叶节点仍须移除或关闭。快照和会话持久化仍支持集合。
...
第一个响应通过 `result` 中的 `{"type":"subscription_started","sequence":N}` 确认订阅。`N` 是请求中所有订阅共同使用的会话全局接受游标。之后的生命周期和自定义事件行在顶层携带会话全局 `sequence`，并按游标顺序合并；游标相同时以请求顺序打破平局。每个基于探测的自定义观测都会预留下一个全局序列，因此快照水位线也能对这些观测排序。生命周期事件订阅从接受游标开始，不会重放此前保留的事件。

Herdr 保留最新 512 个生命周期事件，供事件流追赶。如果订阅方落后过多，导致下一个所需游标已不可用，服务器会写入错误码为 `resync_required` 的响应并关闭该 socket。不要从不完整缓存继续：应打开新订阅，获取新的 `session.snapshot`，并重复上述基于水位线的引导流程。
...
集合事件包括 `collection.created`、`collection.member_added`、`collection.member_moved`、`collection.member_removed`、`collection.member_promoted`、`collection.member_selected`、`collection.members_reordered`、`collection.member_archived`、`collection.member_restored` 和 `collection.closed`。委派事件包括 `delegation.created`、`delegation.reparented`、`delegation.reordered`、`delegation.tombstoned` 和 `delegation.garbage_collected`。可用时变更事件携带变更后的记录;删除/关闭事件携带用于清除缓存的稳定 ID。
...
`TabInfo`、`WorkspaceInfo` 和 `RepositoryInfo` 包含 `descendant_attention_count`。其中 `agent_status` 跟随顶层智能体；该计数表示在顶层父终端成功接收输入之前仍未确认、且位于窗格集合之外的已完成委派后代。当窗格仍是集合成员时，未查看的完成状态只显示在该集合内，并从上层状态和注意力计数中排除；将其提升回平铺窗格后会恢复正常的向上汇总。若委派子项的父窗格仍然存在，向该子项输入不会清除计数。父窗格关闭后，存活的子项会成为确认语义上的有效根。无论是否位于集合中，被阻塞的后代仍会使聚合状态显示为 `blocked`。
