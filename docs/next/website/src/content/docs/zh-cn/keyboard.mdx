...
列表模式用 `up` / `down` 或 `k` / `j` 选择,Space 展开,Enter 进入终端。按已配置的 Herdr 前缀键后再按 Esc,会返回列表且不中断子进程。单独的 Esc 会发送到子 PTY;连续按两次前缀键会把字面前缀发送给子进程。`z` 最大化,`+` / `-` 调高度,`ctrl+up` / `ctrl+down` 调顺序,`a` 归档,`o` 提升,`x` 关闭成员。配置的 `close_pane` 绑定在聚焦集合时也会打开同一流程，`shift+x` 仍是其别名；列表中的单独 `x` / `Delete` 只关闭选中的成员。`shift+x` 选择提升或级联关闭;`shift+a` 在显示数量后清理归档。鼠标可操作行、展开、预览和拖动。
