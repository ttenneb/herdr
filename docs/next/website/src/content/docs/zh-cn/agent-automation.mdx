...
| 集合 | 把多个真实窗格放进一个可滚动叶节点。 |
| 委派 | 独立于位置记录父级、用途和兄弟顺序。 |
...
`idle` 表示智能体正等待输入,且已被前台的人类用户查看。`done` 是未查看工作后的相同底层 idle 状态。普通窗格只有在其标签页实际显示于前台 Herdr 界面时才会标记为已查看。CLI 的 `pane focus`、`agent focus`、读取和等待都不会确认它。`blocked` 表示 Herdr 识别到了审批或提问界面。`unknown` 表示智能体存在,但 Herdr 无法可靠判断其生命周期;它不代表工作成功完成。区别重要时,请指定精确的 `--until` 状态。
...
用集合避免每个辅助智能体占一个顶层分割窗格;用委派让父子关系在移动与归档后保留。
...
复用同一集合,不要从 `done` / `idle` / 退出推断成功,只在验证结果后显式归档。读取、等待、选择和 API 聚焦不会确认子成员。移除时选择保留进程的 `--promote-members` 或关闭全部 PTY 的 `--cascade-close`;不要只因 idle 或 done 就级联关闭。
