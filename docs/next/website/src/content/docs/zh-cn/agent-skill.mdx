...
命令指南就在技能文件本身。它涵盖窗格 ID、`pane split`、`pane run`、`pane read`、`pane wait-output`、`agent wait`、工作区和标签页命令,以及协作配方。

集合编排还应遵循[分组智能体编排](/zh-cn/docs/agent-automation/#分组智能体编排):复用集合,显式创建来源,保持焦点,不要把 `done` / `idle` 当作成功证明,只在结果处理后归档;需要保留终端时选择提升。准确语法以已安装二进制为准。
