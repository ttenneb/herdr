...
cache may be stale. Workspace records add `repository_id` and `checkout`
provenance while retaining legacy `worktree` provenance. Repository records own
checkout ordering, preferred base, and MRU focus. Full Git worktree discovery
