...
`agent.wait` is server-owned and event-driven. It pins the resolved pane occupant so a replacement cannot satisfy the wait. `agent.prompt` accepts an optional `wait` object with `until` and `timeout_ms`; this submits the prompt and starts the wait in one request, avoiding a race between separate calls. A successful `agent.prompt` means the current terminal runtime accepted its serialized text → delay → Enter transaction, not that the PTY will eventually receive it. Each terminal admits at most 8 active-or-queued prompt transactions and 64 KiB of their payloads; `agent_prompt_queue_full` means retry after the runtime drains, while `agent_prompt_payload_too_large` means one transaction exceeds 64 KiB. Runtime close or replacement cancels pending transactions and never writes them to a replacement runtime.
...
stale. Workspace records add `repository_id` and `checkout` provenance while retaining legacy `worktree` provenance. Repository records own checkout ordering, preferred base, and MRU focus. Full Git worktree discovery remains `worktree.list`.
