...
- create, list, focus, rename, move, and close Repositories and Checkouts (`workspace.*` remains compatible)
...
| Repository | `repository.list`, `repository.get`, `repository.focus`, `repository.rename`, `repository.move`, `repository.close` |
| Checkout | `checkout.open`, `checkout.focus`, `checkout.rename`, `checkout.move`, `checkout.close` |
| Workspace compatibility | `workspace.create`, `workspace.list`, `workspace.get`, `workspace.focus`, `workspace.rename`, `workspace.move`, `workspace.move_block`, `workspace.report_metadata`, `workspace.resources.report`, `workspace.close` |
...
| Pane | `pane.split`, `pane.swap`, `pane.move`, `pane.zoom`, `pane.layout`, `pane.process_info`, `pane.neighbor`, `pane.edges`, `pane.focus_direction`, `pane.resize`, `pane.list`, `pane.current`, `pane.get`, `pane.rename`, `pane.send_text`, `pane.send_keys`, `pane.send_input`, `pane.input.set`, `pane.read`, `pane.graphics.info`, `pane.graphics.set`, `pane.graphics.clear`, `pane.graphics.stream`, `pane.report_agent`, `pane.report_agent_session`, `pane.report_metadata`, `pane.clear_agent_authority`, `pane.release_agent`, `pane.close`, `pane.wait_for_output` |
| Collection | `collection.list`, `collection.get`, `collection.create`, `collection.add`, `collection.move`, `collection.promote`, `collection.select`, `collection.reorder`, `collection.archive`, `collection.restore`, `collection.member_create`, `collection.close` |
| Delegation | `delegation.create`, `delegation.get`, `delegation.tree`, `delegation.root`, `delegation.descendants`, `delegation.reparent`, `delegation.reorder` |
| Popup | `popup.close` |
...
`agent.wait` is server-owned and event-driven. It pins the resolved pane occupant so a replacement cannot satisfy the wait. `agent.prompt` accepts an optional `wait` object with `until` and `timeout_ms`; this submits the prompt and starts the wait in one request, avoiding a race between separate calls. If the resolved agent is already `blocked`, `agent.prompt` returns `agent_blocked` without sending input or starting the wait. A successful `agent.prompt` means the current terminal runtime accepted its serialized text → delay → Enter transaction, not that the PTY will eventually receive it. Each terminal admits at most 8 active-or-queued prompt transactions and 64 KiB of their payloads; `agent_prompt_queue_full` means retry after the runtime drains, while `agent_prompt_payload_too_large` means one transaction exceeds 64 KiB. Runtime close or replacement cancels pending transactions and never writes them to a replacement runtime.
...
a `result.snapshot.sequence` watermark, focused workspace/tab/pane ids,
workspace records, tab records, pane records, tab layout snapshots, and agent
records. It is not a subscription. To avoid a bootstrap gap, first open
`events.subscribe` on another connection and wait for the
`subscription_started` acknowledgement. Its `result.sequence` is the one
session-global cursor accepted for every subscription in that request. Buffer
that stream while calling `session.snapshot`, install the snapshot, discard
buffered lines whose top-level `sequence` is at or below the snapshot watermark,
then apply the remaining lines in received order and continue streaming. Each
custom observation reserves a fresh global value, while duplicate lifecycle
subscriptions can repeat the lifecycle event's value. Do not treat `sequence`
as a per-subscription counter or require every stream line to be unique. Call
`session.snapshot` again after reconnecting or when the local cache
may be stale. Workspace records add `repository_id` and `checkout` provenance
while retaining legacy `worktree` provenance. Repository records own checkout
ordering, preferred base, and MRU focus. Full Git worktree discovery remains
`worktree.list`.
...
### Transient workspace resources

Plugins may attach bounded, transient runtime resources to a Checkout or standalone Workspace with `workspace.resources.report`. A report atomically replaces one plugin's complete resource set for that Workspace; an empty array clears it. Resources appear additively in `WorkspaceInfo` and session snapshots, are never persisted, and may expire with `ttl_ms`.

```json
{"id":"jails","method":"workspace.resources.report","params":{"workspace_id":"w1","plugin_id":"hs.jail","resources":[{"resource_id":"immutable-container-id","label":"🔒 jail-a","detail":"/project"}],"seq":4,"ttl_ms":30000}}
```

Shell integrations can avoid JSON argument quoting with `herdr workspace report-resources w1 --plugin hs.jail --file resources.json --ttl-ms 30000` (use `--file -` for stdin).
...
export the tab containing that pane. Tabs containing any collection leaf are
rejected with `layout_contains_collections`. Remove or close each collection
leaf before exporting. For populated collections, you may first promote or move
their members to ordinary tiled panes; member promotion cannot remove an empty
collection leaf. Session snapshots and live handoff still preserve collections.
...
Worktree methods manage Git checkouts inside first-class Repositories. They accept `repository_id`; the older `workspace_id` and `cwd` source selectors remain supported for a migration window. `worktree.create` creates a checkout and returns the new `workspace`, `tab`, `root_pane`, and `worktree` records. If the requested branch already exists locally, it checks out that branch and ignores Base. Otherwise it creates the branch from the requested base. Without an explicit base, repository-scoped creation resolves the stored preferred base, `origin/HEAD`, `main`/`master`, then the MRU Checkout’s `HEAD`. `worktree.open` opens an existing checkout or returns the already-open workspace. `worktree.remove` runs `git worktree remove` against a linked child workspace and never deletes the branch.
...
All four raw worktree methods accept `trust_repository`, which defaults to `false` when omitted. Leave it false unless Git rejects the selected repository for unsafe or dubious ownership. After independently verifying the resolved repository path, retry the same request with `"trust_repository": true`. Trust is request-scoped: Herdr adds `git -c safe.directory=<resolved-repository>` only to Git subprocesses for that request. It does not persist trust, edit Git configuration, or trust any other path. Every later request defaults to `false` again and must opt in separately.
...
## Collections and delegation

Pane collections use `collection.list`, `collection.get`, `collection.create`,
`collection.add`, `collection.move`, `collection.promote`, `collection.select`,
`collection.reorder`, `collection.archive`, `collection.restore`,
`collection.member_create`, and `collection.close`. A non-empty close requires
`disposition: "cascade_close"` or `"promote_members"`. Cascade close terminates
every member PTY; promote members preserves every process in its own new
standalone tab in the same workspace. `target_pane_id` must be omitted when
promoting a collection. Collection responses include stable IDs, ordered members, archive/selection state,
lifecycle counts, and advisory warnings. For delegated members,
`collection.reorder.index` is the requested final canonical displayed index of
the moved subtree root. The moving subtree is removed before that index is
interpreted, and the target must be a sibling-subtree boundary in the same
active/archive section under the same delegation parent. Use
`delegation.reparent` to change parentage. Nondelegated members retain direct
member-vector reordering.

Delegation metadata is independent of placement. Use `delegation.create`,
`delegation.get`, `delegation.tree`, `delegation.root`,
`delegation.descendants`, `delegation.reparent`, and `delegation.reorder` to
record/query parentage, purpose, derived roots, and sibling order. Reparenting
rejects cycles atomically.

`PaneInfo.placement` and each `PaneLayoutPane.placement` are either `tiled` or
`collection` with `collection_id`, `member_index`, `archived`, and `selected`.
`PaneLayoutSnapshot.collections` describes collection leaves, while `focused`
is typed as a pane or collection (with an optional selected pane).
`focused_pane_id` remains optional for compatibility with empty collections.
Portable layout roots do not support collection leaves. `layout.export` rejects
both empty and populated collections rather than silently dropping collection
structure or members. Every collection leaf must be removed or closed before
exporting; populated collections may first promote or move their members, while
empty leaves still require removal or closure. Snapshot and session persistence
remain collection-aware.
...
The first response acknowledges the subscription as
`{"type":"subscription_started","sequence":N}` inside `result`. `N` is the
common session-global acceptance cursor for every requested subscription.
Later lifecycle and custom-event lines carry their session-global `sequence` at
the top level and are merged in cursor order; request order breaks ties. Each
probe-backed custom observation reserves the next global sequence, so snapshot
watermarks also order those observations. Lifecycle subscriptions start at the
accepted cursor and do not replay events
retained before it.

Herdr retains the latest 512 lifecycle events for a stream to catch up. If a
subscriber falls far enough behind that the next required cursor is no longer
available, the server writes an error response with code `resync_required` and
closes that socket. Do not continue from the partial cache: open a fresh
subscription, obtain a new `session.snapshot`, and repeat the watermark-based
bootstrap procedure above.

Repository event subscriptions include `repository.created`, `repository.renamed`, `repository.moved`, `repository.closed`, and `repository.focused`. Checkout lifecycle subscriptions include `checkout.opened`, `checkout.renamed`, `checkout.moved`, `checkout.closed`, and `checkout.focused`; each is emitted as an additive alias after its compatibility `workspace.*` event. Repository creation precedes the first child `workspace.created`; closing the final Checkout emits `workspace.closed`, `checkout.closed`, then `repository.closed`.

Workspace event subscriptions include `workspace.created`, `workspace.updated`, `workspace.metadata_updated`, `workspace.resources_updated`, `workspace.renamed`, `workspace.moved`, `workspace.closed`, and `workspace.focused`. `workspace.resources_updated` reports replacement and TTL expiry of transient plugin-owned Workspace Resources. `workspace.metadata_updated` reports token changes and TTL expiry without invoking plugin event hooks. Other workspace events describe Herdr UI/runtime lifecycle. `workspace.created` includes optional `workspace.worktree` provenance when the workspace belongs to a worktree group. `workspace.moved` includes the moved `workspace_id`, requested `insert_index`, and updated ordered `workspaces` list. `workspace.closed` includes a final `workspace` snapshot when Herdr can still identify it before removal.
...
Collection subscriptions are `collection.created`, `collection.member_added`,
`collection.member_moved`, `collection.member_removed`,
`collection.member_promoted`, `collection.member_selected`,
`collection.members_reordered`, `collection.member_archived`,
`collection.member_restored`, and `collection.closed`. Delegation subscriptions
are `delegation.created`, `delegation.reparented`, `delegation.reordered`,
`delegation.tombstoned`, and `delegation.garbage_collected`. Mutation events
carry the resulting collection/delegation where available; removal and closure
events carry the stable IDs needed to invalidate cached records.
...
`TabInfo`, `WorkspaceInfo`, and `RepositoryInfo` include `descendant_attention_count`. Their `agent_status` follows top-level agents; this count reports completed delegated descendants outside pane collections that have not yet been acknowledged by successful terminal input to their top-level parent. Unseen completion stays local while a pane is a collection member and is excluded from ancestor status and attention counts; promoting it back to a tiled pane restores normal rollup behavior. Input to a delegated child with a live parent does not clear the count. If the parent pane has closed, the surviving child becomes an effective root for acknowledgement. Blocked descendants continue to make the aggregate status `blocked` regardless of collection placement.
