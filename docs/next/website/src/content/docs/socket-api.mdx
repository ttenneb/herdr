...
`TabInfo`, `WorkspaceInfo`, and `RepositoryInfo` include `descendant_attention_count`. Their `agent_status` follows top-level agents; this count reports completed delegated descendants that have not yet been acknowledged by entering their terminal. Blocked descendants continue to make the aggregate status `blocked`.
