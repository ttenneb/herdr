...
`shift+x` shows lifecycle counts and asks whether to promote all members or cascade-close every PTY. `shift+a` shows archived live/exited/working/blocked counts before destructive archive cleanup. Archiving itself is non-destructive.
