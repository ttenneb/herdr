...
A focused collection starts in **list mode**. Use `up`/`down` or `k`/`j` to select, Space to expand, and Enter to enter the selected terminal. The configured Herdr prefix returns to the list without interrupting the child. Bare Esc is sent to the child PTY. Use `z` to maximize within the collection, `+`/`-` to resize a preview, `ctrl+up`/`ctrl+down` to reorder, `a` to archive or restore, `o` to promote, and `x` to close a member.
