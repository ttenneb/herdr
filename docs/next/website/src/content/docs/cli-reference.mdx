...
Repositories and Checkouts are the preferred names for Git-backed Spaces:

```bash
herdr repository list [--json]
herdr repository get <repository_id> [--json]
herdr repository focus <repository_id>
herdr repository rename <repository_id> <label>
herdr repository move <repository_id> <insert_index>
herdr repository close <repository_id>
herdr checkout open [--cwd PATH] [--label TEXT] [--focus]
herdr checkout focus <workspace_id>
herdr checkout rename <workspace_id> <label>
herdr checkout move <workspace_id> <insert_index>
herdr checkout close <workspace_id>
```

`repository close` closes every child Checkout after explicit repository targeting; it never removes directories or branches. `checkout close` closes one Checkout. The legacy workspace commands remain available for scripts:
...
herdr workspace report-resources <workspace_id> --plugin ID --file <json-file|-> [--seq N] [--ttl-ms N]
...
A workspace is the compatibility name for a Checkout or standalone Space. Opening a path inside an existing Git checkout reuses that Checkout, and Git checkouts sharing a canonical common directory group automatically. Creating one also creates its first tab and root pane. The JSON response exposes their IDs as `.result.workspace.workspace_id`, `.result.tab.tab_id`, and `.result.root_pane.pane_id`.

`workspace report-resources` accepts a JSON array of plugin-owned `{resource_id,label,detail?,data?}` records. It atomically replaces that plugin's transient resource set for the Workspace; resources are bounded, may use TTL expiry, and are not saved in session state.
...
Worktrees open as peer Checkouts in a Repository. `worktree create` creates a linked checkout and opens it in the matching Repository. If `--branch` names an existing local branch, Herdr checks it out and ignores Base; otherwise it creates the branch from the visible `--base`. Repository-scoped creation defaults through the stored preferred base, `origin/HEAD`, `main`/`master`, then the most recently used Checkout’s `HEAD`. Without `--path`, Herdr creates the checkout under `<worktrees.directory>/<repo>/<branch-slug>`.
...
```bash
herdr collection list [--workspace ID] [--tab ID]
herdr collection get <collection_id>
herdr collection create --target-pane ID --direction right|down [--ratio FLOAT] [--label TEXT] [--focus|--no-focus]
herdr collection add <collection_id> <tiled_pane_id>
herdr collection move <member_pane_id> <collection_id>
herdr collection member-create <collection_id> [--cwd PATH] [--env KEY=VALUE] [--parent DELEGATION_ID] [--purpose TEXT]
herdr collection select <collection_id> <pane_id> [--focus|--no-focus]
herdr collection reorder <collection_id> <pane_id> --index N
herdr collection archive <collection_id> <pane_id>
herdr collection restore <collection_id> <pane_id>
herdr collection promote <pane_id> --target-pane ID --direction right|down [--ratio FLOAT] [--focus|--no-focus]
herdr collection close <collection_id> [--cascade-close|--promote-members] [--focus-promoted]
```

Members remain independently addressable panes and PTYs. `add` collects a tiled pane; `move` transfers an existing member. `member-create` returns `.result.created.pane.pane_id` and an optional `.result.created.delegation_id`; start its agent separately. Creation and movement preserve focus unless requested. Selection and API focus do not enter or mark a grouped terminal seen.

Archive is live and reversible. Input, `working`, or `blocked` restores a member; reads, observation, `idle`, and process exit do not. Lifecycle thresholds only warn. A non-empty close requires `--cascade-close` to close every member PTY or `--promote-members` to preserve every process in its own standalone tab. `--focus-promoted` focuses the first promoted member.

## Delegations

```bash
herdr delegation create [--pane ID] [--parent ID] [--purpose TEXT]
herdr delegation get <delegation_id>
herdr delegation tree
herdr delegation root <delegation_id>
herdr delegation descendants <delegation_id>
herdr delegation reparent <delegation_id> (--parent ID|--root)
herdr delegation reorder <delegation_id> (--first|--last|--before ID|--after ID)
```

Delegation IDs such as `d1` are stable session provenance, separate from pane and collection IDs. Parent updates reject missing parents, self-parenting, and cycles. `reorder` changes sibling order; `reparent` changes relationships. Parent pane closure leaves a tombstone while descendants exist. Placement, movement, and archival do not rewrite delegation.

Socket methods use the corresponding `collection.*` and `delegation.*` names. Run `herdr api schema --json` for exact requests, results, placement/focus fields, warnings, and events.
...
`idle` means ready and seen; `done` is the same underlying idle state after unseen work. A tiled pane is acknowledged only when its tab is actually viewed by the foreground Herdr UI. CLI `pane focus` and `agent focus` change focus but do not acknowledge attention. A collection member is acknowledged only when a foreground human enters its terminal; focus only selects it. CLI reads and waits never acknowledge attention. `blocked` is a recognized approval/question UI, and `unknown` does not prove success.
